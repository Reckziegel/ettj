#' Estimates the 5 factors and 3 decay parameters in a Svensson (1994) like factor model. The decay parameters are optimized using the two step approach.
#'
#' @description
#' Determines the optimal decay parameters calling the rmse function.
#' Then, performs an OLS regression using the 5 factor yield curve model in order to determine the optimal \eqn{\lambda} decay parameters:
#' \deqn{y_{\tau} = \beta_{1} +
...
#'
#' @param lambda Initial guess for the 3 decay parameters. Numeric values.
...
#'
#' @return A matrix containing the 5 factors and 3 decay parameters, respectively.
#' @export
#'
#' @examples
#' 
...
factors_5f <- function(lambda, yields, maturidades){
  ...
  lam3 <- optlamfun$par[3]
  
  # Estimating Betas --------------------------------------------------------
  
  
  H <-  matrix(NA,length(yields),5)
  B <-  matrix(NA,5,1)
  ...
  H[1:length(yields),5] <- (1-exp(-t(maturidades)*lam3))/(t(maturidades)*lam3) - exp(-t(maturidades)*lam3)
